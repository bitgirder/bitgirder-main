...
type unreadElt struct {
    elt stackElt
    synthLoc *loc.Location
}

// Will at some point add Lexer.SetSourceName( string ) to allow callers to set
// value of loc.Source
type Lexer struct {
    reader *bufio.Reader
    SourceName string
...
func ( lx *Lexer ) makeLocation() *loc.Location {
    return &loc.Location{ 
...
func ( lx *Lexer ) Location() *loc.Location { 
    if lx.stackEmpty() { return lx.makeLocation() }
    return lx.peek().lc
}

func ( lx *Lexer ) makeParseError( 
    adj int, fmtStr string, args ...interface{} ) *loc.ParseError {
    l := lx.makeLocation()
    l.Col += adj
    return &loc.ParseError{ fmt.Sprintf( fmtStr, args... ), l }
}
...
func isKeyword( id Identifier ) ( kwd Keyword, ok bool ) {
    if len( id ) == 1 { 
        if kwd, ok = kwdMap[ string( id[ 0 ] ) ]; ok { return } 
    }
    return
}
...
func ( lx *Lexer ) readIdentifier() ( id Token, err error ) {
    ...
    if ( ! lx.isExternal ) { 
        if kwd, ok := isKeyword( id.( Identifier ) ); ok { id = kwd } 
    }
    return 
}
...
//
func ( lx *Lexer ) readStack() ( tok Token, lc *loc.Location, err error ) {
    elt := lx.peek()
...
func ( lx *Lexer ) recordEof(
    tok Token, lc *loc.Location, err error ) ( Token, *loc.Location, error ) {
    lx.sawEof = true
    if tok == nil {
        if lx.synthLoc != nil {
...
func ( lx *Lexer ) doStrip() ( 
    eol bool, tok Token, lc *loc.Location, err error ) {
...
func ( lx *Lexer ) callTokenRead( 
    f tokCall, lc *loc.Location ) ( Token, *loc.Location, error ) {
    tok, err := f()
...
func ( lx *Lexer ) updateSynthLoc( tok Token ) {
    setLoc := false
    switch v := tok.( type ) {
    case Identifier, DeclaredTypeName, StringToken, *NumericToken: setLoc = true
...
func ( lx *Lexer ) implReadToken( 
    f tokCall ) ( tok Token, lc *loc.Location, err error ) {
...
func ( lx *Lexer ) ReadToken() ( Token, *loc.Location, error ) {
...
func ( lx *Lexer ) ReadIdentifier() ( Token, *loc.Location, error ) {
    return lx.implReadToken( func() ( Token, error ) {
        return lx.readIdentifier()
    })
}

func ( lx *Lexer ) ReadDeclaredTypeName() ( Token, *loc.Location, error ) {
    return lx.implReadToken( func() ( Token, error ) {
        return lx.readDeclaredTypeName()
    })
}

func ( lx *Lexer ) ReadNumber() ( Token, *loc.Location, error ) {
    return lx.implReadToken( func() ( Token, error ) {
