...
func parseToken( 
    in string, t *testing.T ) ( tok Token, loc *loc.Location, err error ) {
...
func parseCoreParseTest( 
    cpt *pt.CoreParseTest, t *testing.T ) ( tok Token, err error ) {
    a := newLexerAsserter( cpt.In, false, t )
    switch cpt.TestType {
    case pt.TestTypeString: tok, _, err = a.lx.ReadToken()
    case pt.TestTypeNumber: 
        if tok, _, err = a.lx.ReadToken(); err == nil {
            if tok == SpecialTokenMinus && cpt.Expect != nil {
                assert.True( cpt.Expect.( *pt.NumericToken ).Negative )
...
func convCptVal( val interface{} ) Token {
    switch v := val.( type ) {
    case pt.StringToken: return StringToken( v )
    case *pt.NumericToken:
        expRune := rune( 0 )
...
func assertCoreParseError( cpt *pt.CoreParseTest, err error, t *testing.T ) {
    if cpt.Err == nil { t.Fatal( err ) }
    if pe, ok := err.( *loc.ParseError ); ok {
        ee := cpt.Err.( *pt.ParseErrorExpect )
...
func assertCoreParse( cpt *pt.CoreParseTest, t *testing.T ) {
    if tok, err := parseCoreParseTest( cpt, t ); err == nil {
//    if tok, _, err := parseToken( cpt.In, t ); err == nil {
        if cpt.Err == nil { 
...
func TestCoreParseTests( t *testing.T ) {
    for _, cpt := range pt.CoreParseTests {
        if cpt.TestType == pt.TestTypeString || 
           cpt.TestType == pt.TestTypeNumber { 
...
func TestTokenVariations( t *testing.T ) {
    f := func( s string, expct Token ) {
        if tok, l, err := parseToken( s, t ); err == nil {
            assert.Equal( 
                &loc.Location{ 1, 1, loc.ParseSourceInput }, l )
            assert.Equal( expct, tok )
        } else { t.Fatal( err ) }
    }
    f( "x", id( "x" ) )
...
func TestExternalIdsNotAsKeywords( t *testing.T ) {
    ...
    lx = New( opts )
    if tok, _, err := lx.ReadToken(); err == nil {
        assert.Equal( id( "namespace" ), tok.( Identifier ) )
    } else { t.Fatal( err ) }
}
