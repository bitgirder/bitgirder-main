...
import com.bitgirder.lang.AbstractTypedString;
...
class Sql
{
    ...
    static
    void
    checkPublicPrepareArgs( Connection conn,
                            CharSequence stmt,
                            Object... bindArgs )
    {
    ...
    prepareAndBind( Connection conn,
        ...
        throws SQLException
    {
        checkPublicPrepareArgs( conn, stmt, bindArgs );

        PreparedStatement st = conn.prepareStatement( stmt.toString() );
        return bindSafe( st, bindArgs );
    }
        ...
        throws SQLException
    {
        ...
        PreparedStatement st = 
            conn.prepareStatement( stmt.toString(), autogeneratedKeys );
    ...
    // only used internally for now, can be made public when needed
    private
    static
    interface ResultSetUser< V >
    {
        public
        V
        useResult( ResultSet rs )
            throws Exception;
    } 
    ...
    // Could be exposed later if needed, or wrapped with additional friendlier
    // frontends as needed
    private
    static
    < V >
    V
    insertAndGetKeys( Connection conn,
                      ResultSetUser< V > user,
                      CharSequence stmt,
                      Object... bindArgs )
        throws Exception
    {
        PreparedStatement st = 
            prepareAndBind( 
                conn, stmt, Statement.RETURN_GENERATED_KEYS, bindArgs );
        ...
        try
        {
            ...
            try { return user.useResult( rs ); } finally { rs.close(); }
        }
        finally { st.close(); }
    }
    ...
    insertAndGetKeys( Connection conn,
        ...
        throws SQLException
    {
        ResultSetUser< Map< String, Object > > u =
            new ResultSetUser< Map< String, Object > >() {
                public Map< String, Object > useResult( ResultSet rs )
                    throws SQLException
                {
                    if ( rs.next() ) 
                    {
                        return rowToMap( rs, getColumnLabels( rs ) );
                    }
                    else return Lang.emptyMap();
                }
            };

        try { return insertAndGetKeys( conn, u, stmt, bindArgs ); }
        catch ( Exception ex ) { throw createSqlRethrow( ex ); }
    }
    ...
    < V >
    V
    insertAndGetKey( Connection conn,
                     final Class< V > cls,
                     CharSequence stmt,
                     Object... bindArgs )
        throws SQLException
    {
        ResultSetUser< V > u = new ResultSetUser< V >() {
            public V useResult( ResultSet rs ) throws Exception 
            {
                if ( ! rs.next() ) return null;
                return getObject( rs, COL_GENERATED_KEY, cls );
            }
        };
        ...
        throws Exception
    {
        ...
        return useConnection( ds.getConnection(), user );
    }
    ...
    < V >
    ConnectionUser< V >
    asTransactionUser( final ConnectionUser< V > u )
    {
        inputs.notNull( u, "u" );

        return new ConnectionUser< V >() {
            public V useConnection( Connection conn ) throws Exception
            {
                boolean save = conn.getAutoCommit();
                try
                {
                ...
                finally { conn.setAutoCommit( save ); }
            }
        };
    }
    ...
    < V >
    V
    useTransaction( Connection conn,
                    ConnectionUser< V > u )
        throws Exception
    {
        inputs.notNull( u, "u" );
        return useConnection( conn, asTransactionUser( u ) );
    }
    ...
    < V >
    V
    useTransaction( DataSource ds,
                    ConnectionUser< V > u )
        throws Exception
    {
        inputs.notNull( u, "u" );
        return useConnection( ds, asTransactionUser( u ) );
    }
}
